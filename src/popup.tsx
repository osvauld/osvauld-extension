    ...
    <div className="w-400 h-380 flex flex-col justify-around items-center bg-[#262C44]">
      <div className="name-with-icon w-11/12 flex self-center pl-4">
         ...
         <div className="body w-11/12 h-4/6">
          {loginStatus ? ( credentialList ? (
             credentialSelected ? (<CredView  chageToList={chageToList}/>):( <CredsListed  updateCredentialSelected={updateCredentialSelected}/>)
          ) : ( <EmptyCreds /> )):(<LoginView loginAction={loginAction}/>)}
         </div>
    </div>
  )
}
