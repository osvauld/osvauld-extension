...
const CredsListed = ({ updateCredentialSelected }) => {
   ...
   const changeCredentialSelected = () => {
    ...
    const storage = new Storage()
      ...
      useEffect(()=>{
         (async ()=>{
            const list = await storage.get('usernames');
            console.log('reading plasmo store as received in login', list);
            if (list.length > 0) {
